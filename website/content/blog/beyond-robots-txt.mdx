...
APoP is an open standard specifically designed for AI agent authorization. Unlike robots.txt, which was built for crawlers, APoP handles the full range of agent interactions through [structured JSON policies](/docs/introduction/quick-start) with cryptographic verification.

The **Agent Policy Protocol (APoP)** is an [open standard](/docs/introduction/what-is-apop) designed to complement `robots.txt` for the agentic web. While `robots.txt` handles crawl control, APoP handles agent authorization — using a structured JSON document placed at `/.well-known/agent-policy.json`.

Learn more in our [quick start guide](/docs/introduction/quick-start) or explore [real-world examples](/docs/examples/news-publisher).
...
- **Identity requirements** — require [agents to identify themselves](/docs/specification/agent-identification) with cryptographic verification
- **Rate limits** — per-path, per-agent request limits
- **Action vocabulary** — read, summarize, extract, train, navigate, execute, [and more](/docs/specification/schema-reference#actions)
...
**[Read the APoP specification →](/docs/specification/overview)**

**[Try the interactive playground →](/playground)**

**[See implementation guides →](/docs/guides/nextjs)**
