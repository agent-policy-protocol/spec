...
### 3. Serve the policy (optional)

Place `agent-policy.json` in your static files or serve it via a view:
...
| `APOP_POLICY`          | `dict`       | —       | Inline policy dict (alternative to file) |
| `APOP_SKIP_NON_AGENTS` | `bool`       | `True`  | Skip enforcement for non-agent requests  |

One of `APOP_POLICY_FILE` or `APOP_POLICY` is required.
...
def agent_policy_view(request):
    ...
    if result.valid and result.policy:
        # Serialize the policy dataclass to a dict
        from dataclasses import asdict
        return JsonResponse(asdict(result.policy))
    return JsonResponse({})
```
